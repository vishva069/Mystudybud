...
class Auth {
     ...
     */
    public function updateUserToTutor($userId) {
        try {
            $stmt = $this->db->prepare(
                "UPDATE users SET role = 'tutor' WHERE id = :id"
            );
     ...
     */
    public function isTutorOrAdmin() {
        if (!$this->isLoggedIn()) {
            return false;
        }
        
        return isset($_SESSION['role']) && ($_SESSION['role'] === 'tutor' || $_SESSION['role'] === 'admin' || $_SESSION['role'] === 'instructor');
    }
